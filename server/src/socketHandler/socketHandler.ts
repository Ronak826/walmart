...
export const socketHandler = (io: Server, socket: Socket) => {
  ...
  socket.on("send-help-request", async ({ helpRequestId}) => {
    try {
      ...
      socket.broadcast.emit("receive-help-request", {
        ...
        driverno:helpRequest.requester?.phoneNo,
        driverphoto:helpRequest.requester?.driverimage,
        vehicleno:helpRequest.requester?.vehicleNo,
  ...
  try {
    ...
    socket.broadcast.emit("help-accepted", {
      helpRequestId: helpRequest.id,
      helperId: helperId,
      driverno:helpRequest.helper?.phoneNo,
      driverphoto:helpRequest.helper?.driverimage,
      vehicleno:helpRequest.helper?.vehicleNo,
